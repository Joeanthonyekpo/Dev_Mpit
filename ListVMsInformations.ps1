...
$WindowsRunningVMs = get-azvm -status 
...
foreach ($vm in $WindowsRunningVMs) {
    ...
    if ($null -ne $VMNetworkInterfaces) {
        foreach($nic in $VMNetworkInterfaces) {
            ...
            $PrivateIPList = ""
            $PublicIPList = ""
            foreach ($PubIPConfig in $CurrentNic.IpConfigurations) {
                $PrivateIPList = $PrivateIPList + " " + $PubIPConfig.PrivateIpAddress
                if ($null -ne $PubIPConfig.PublicIpAddress) {
                    $CurrentPublicIP = (Get-AzPublicIpAddress -Name CIF-mgmt-RDS-ip -ResourceGroupName RG_MGMT).IpAddress
